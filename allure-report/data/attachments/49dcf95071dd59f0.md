...
```
Error: locator.waitFor: Error: strict mode violation: locator('a[href*=\'apply\'], button:has-text(\'Apply\')') resolved to 2 elements:
    1) <a class="" role="menuitem" href="/careers/#search-and-apply">Search and Apply</a> aka getByText('Search and Apply')
    2) <a role="button" target="_blank" class="square square-inverted" href="https://gdit.wd5.myworkdayjobs.com/External_Career_Site/job/USA-NY-Albany/Test-Engineer-Senior--Manual-_RQ219281-1/apply">Apply Now</a> aka getByRole('button', { name: 'Apply Now' })

Call log:
  - waiting for locator('a[href*=\'apply\'], button:has-text(\'Apply\')') to be visible
    3 × locator resolved to hidden <a class="" role="menuitem" href="/careers/#search-and-apply">Search and Apply</a>
...
```ts
  ...
  2  | 
  3  | export class ApplyPage extends BasePage {
  4  | 
  5  |   applyBtn = this.page
  6  |     .locator("a[href*='apply'], button:has-text('Apply')");
  7  | 
  8  |   async clickApplyButton() {
> 9  |     await this.applyBtn.waitFor({ state: 'visible' });
     |                         ^ Error: locator.waitFor: Error: strict mode violation: locator('a[href*=\'apply\'], button:has-text(\'Apply\')') resolved to 2 elements:
  10 |     await this.click(this.applyBtn);
  11 |   }
  12 | }
```